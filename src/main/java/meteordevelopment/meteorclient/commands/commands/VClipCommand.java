        ...
        builder.then(argument("blocks", DoubleArgumentType.doubleArg()).executes(context -> {
            ...
            if (mc.player.hasVehicle()) {
                ...
                for (int packetNumber = 0; packetNumber < (packetsRequired - 1); packetNumber++) {
                    mc.player.networkHandler.send(new VehicleMoveC2SPacket(mc.player.getVehicle()));
                }
                ...
                mc.player.networkHandler.send(new PlayerMoveC2SPacket.PositionAndOnGround(mc.player.getX(), mc.player.getY() + blocks, mc.player.getZ(), true));

    ...
    private void move(double velX, double velY, double velZ) {
        switch (moveType.get()) {
            ...
            case Packet -> {
                ...
                mc.player.networkHandler.send(new PlayerMoveC2SPacket.PositionAndOnGround(newPos.x, newPos.y, newPos.z, false));
                mc.player.networkHandler.send(new PlayerMoveC2SPacket.PositionAndOnGround(newPos.x, newPos.y - 0.01, newPos.z, true));
            }
        }
    }
